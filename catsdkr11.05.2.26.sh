...
#
#   WSL2-oriented. devkitPro: official installer (wget -U dkp-apt) — no GitHub,
...
is_wsl2() {
    ...
    grep -qiE 'WSL2|microsoft-standard-WSL' /proc/sys/kernel/osrelease 2>/dev/null
}
...
#-------------------------------------------------------------------------------
#  devkitPro pacman (no direct .deb URL — Cloudflare 403; use official script + UA)
#-------------------------------------------------------------------------------
install_devkitpro_pacman() {
    [[ -x "${DKP_PM}" ]] && return 0
    echo -e "${C}Installing devkitPro pacman (wget -U dkp-apt)…${RST}"
    if [[ ! -e /etc/mtab ]]; then
        sudo ln -sf /proc/self/mounts /etc/mtab || true
    fi
    local scr="${HOME}/retro-dev/install-devkitpro-pacman"
    rm -f "$scr"
    ...
    chmod +x "$scr"
    # Official script runs apt; must be noninteractive or it stops at [Y/n]
    sudo env DEBIAN_FRONTEND=noninteractive NEEDRESTART_MODE=a bash "$scr"
    rm -f "$scr"
}
...
if ((${#DKP_PKGS[@]})); then
    echo -e "\n${C}Installing optional devkit groups:${RST} ${DKP_PKGS[*]}"
